...
function startTeleboard() {
	...
	pm2.connect((err: any) => {
		...
		pm2.start(mainFile, (err: any, proc: any) => {
			if(err) {
			...
			pm2.describe((proc as any)[0]['pm_id'], (err: any, desc: any) => {
				if(handleErr(err)) return;
...
function runTeleboard() {
	...
	teleboard.options.isInCLI = false;
	teleboard.initLogger();
	teleboard.launch();
